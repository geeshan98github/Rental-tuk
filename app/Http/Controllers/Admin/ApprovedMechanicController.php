...
class ApprovedMechanicController extends Controller
{
    public static function middleware(): array
    {
        return [new Middleware('approved-mechanic-list', only: ['list', 'view'])];
    }
    ...
    public function list(Request $request)
    {
        ...
        if ($request->ajax()) {
            if ($role == 'Admin') {
                $data = Mechanic::where('is_delete', 0)->where('approve_status', 'approved')->orderBy('created_at', 'asc');
            } elseif ($role == 'Branch Manager') {
                $data = DB::table('applicants_for_mechanic')->join('tbl_city', 'tbl_city.id', '=', 'applicants_for_mechanic.city_id')->where('tbl_city.branch_id', $user_branch)->where('applicants_for_mechanic.is_delete', 0)->where('applicants_for_mechanic.approve_status', 'approved')->select('applicants_for_mechanic.*')->orderBy('applicants_for_mechanic.created_at', 'asc');
            } else {
        ...
        return view('admin.mechanic.approved_list');
    }
}