...
class InstructorApplicantController extends Controller
{
    public static function middleware(): array
    {
        return [new Middleware('instructor-applicant-list', only: ['list', 'view'])];
    }
    ...
    public function sendApprovel(Request $request)
    {
        ...
        return redirect()->route('instructor-applicant-list')->with('success', 'Send to Approval Successfully');
    }

    public function action(Request $request)
    {
        $id = $request->id;
        return view('admin.instructor.action', compact('id'));
    }
    ...
    public function saveAction(Request $request)
    {
        ...
        if ($request->action == 'Approve') {
            ...
            return redirect()->route('instructor-applicant-list')->with('success', 'Approved Successfully');
        }
        if ($request->action == 'Reject') {
            $data->update([
                'approve_status' => 'rejected',
            ]);
            return redirect()->route('instructor-applicant-list')->with('success', 'Rejected Successfully');
        }
    }
}
